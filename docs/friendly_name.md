...
Both commands are subcommands of `fit-update`. Each writes `friendly_name` to every configured market's local pyturso replica, then `push()`es that replica to Turso — regardless of any `--market`/`--north` flag passed.

### update-friendly-name

Set a friendly display name for all fits in a doctrine by doctrine ID, across every configured market.
...
**Behavior:**
- Updates all rows in `doctrine_fits` where `doctrine_id = <id>`, on every configured market
- Pushes each market's replica to Turso after writing it

### populate-friendly-names

Bulk populate `friendly_name` values from a JSON file, across every configured market.
...
- Writes and pushes each configured market's replica in turn
...
- **Backend utilities:** `ensure_friendly_name_column()`, `update_doctrine_friendly_name()`, `populate_friendly_names_from_json()` in `src/mkts_backend/utils/doctrine_update.py`
